...
describe("hooks/codex/sessionstart.mjs — rule capture", () => {
  ...
  test("captures project AGENTS.md and AGENTS.override.md as rule events on startup", () => {
    writeFileSync(
    ...
    writeFileSync(
      join(fakeProject, "AGENTS.override.md"),
      "# Project overrides\n\nPrefer scoped Hindsight banks.\n",
      "utf8",
    );
    ...
    expect(rows).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({ type: "rule", data: join(fakeProject, "AGENTS.override.md") }),
        ...
        expect.objectContaining({
          type: "rule_content",
          data: expect.stringContaining("Prefer scoped Hindsight banks."),
        }),
      ]),
    );
  });
